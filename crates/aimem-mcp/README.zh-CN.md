...
- `codex_*` 工具是面向 Codex 类 agent 的私有编码经验层：把已编辑 repo profile、紧凑 experience card 和任务上下文写入同一个本地 AiMem DB。`codex_record_repo` 只围绕显式传入的 `repo_path` 做保守检测（repo root、已知 manifest、可能的 test commands、`.git` HEAD/origin）。它们**不会**自动爬取文件系统，也不会自动导入完整对话 transcript。
